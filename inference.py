...
def main():
    ...
    nfnet_outcomes = load_model_inference(
        PATH_NFNET, df, MODEL_NFNET, "eca_nfnet_l0", device
    )
    effnet_outcomes = load_model_inference(
        PATH_EFFNET, df, MODEL_EFFNET, "tf_efficientnet_b3_ns", device
    )
    vit_outcomes = load_model_inference(
        PATH_ViT, df, MODEL_ViT, "vit_base_patch16_384", device
    )

    final_preds = (
        effnet_outcomes_b3.drop("ImageID", axis=1) * 0.6
        + nfnet_outcomes.drop("ImageID", axis=1) * 0.3
        + vitnet_outcomes.drop("ImageID", axis=1) * 0.1
    ).to_numpy()
