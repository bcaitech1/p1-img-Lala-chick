...
def train_one_epoch(epoch, scaler, model, loss_fn, optimizer, train_loader, old_train_loader, device, scheduler, cfg):
    model.train()
...
def main(cfg):
    ...
    for fold in range(cfg['fold_num']):
        ...
        if cfg['base'] == 'efficient':
            model = MaskEfficientNet(cfg['model'], train.label.nunique(), pretrained=True).to(device)
        elif cfg['base'] == 'nfnet':
            model = MaskNFNet(cfg['model'], train.label.nunique(), pretrained=True).to(device)
        elif cfg['base'] == 'ViT':
            model = MaskViT(cfg['model'], train.label.nunique(), pretrained=True).to(device)
        ...
        for epoch in range(cfg['epochs']):
            train_one_epoch(epoch, scaler, model, loss_fn, optimizer, train_loader, old_train_loader, device, scheduler, cfg)

            with torch.no_grad():
                epoch_acc, epoch_f1 = valid_one_epoch(epoch, model, loss_fn, val_loader, device)
